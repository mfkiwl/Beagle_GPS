...
ifeq ($(OPT),O0)
	DIRS = . pru $(PKGS) web
	DIRS += platform/$(PLATFORM) rx rx/CuteSDR rx/csdr gps ui support arch arch/$(ARCH)
else
	DIRS = . pru $(PKGS) web
endif
...
ifeq ($(OPT),O0)
	...
	DIRS_O3 = platform/$(PLATFORM) rx rx/CuteSDR rx/csdr gps ui support arch arch/$(ARCH)
endif
...
REMOVE = $(subst web/web.c,,$(subst web/edata_embed.c,,$(subst web/edata_always.c,,$(C))))
...
ALL_DEPS += $(GEN_ASM) $(OUT_ASM) $(CMD_DEPS) $(GEN_NOIP2)
...
EDATA_DEP = web/kiwi/Makefile web/openwebrx/Makefile web/pkgs/Makefile
...
kiwi.bin: c_ctr_reset $(OBJ_DIR) $(OBJ_DIR_O3) $(KEEP_DIR) $(OBJECTS) $(O3_OBJECTS) $(BIN_DEPS) $(DEVEL_DEPS)
	@echo $(C_CTR_LINK) >.comp_ctr
	g++ $(OBJECTS) $(O3_OBJECTS) $(DEVEL_DEPS) $(LIBS) -o $@

kiwid.bin: c_ctr_reset $(OBJ_DIR) $(OBJ_DIR_O3) $(KEEP_DIR) $(OBJECTS) $(O3_OBJECTS) $(BIN_DEPS) $(EMBED_DEPS)
	@echo $(C_CTR_LINK) >.comp_ctr
...
EXCLUDE_RSYNC = ".git" "/obj" "/obj_O3" "/obj_keep" "*.dSYM" "*.bin" "*.aout" "e_cpu/a" "*.aout.h" "kiwi.gen.h" "verilog/kiwi.gen.vh" "web/edata*.c" ".comp_ctr" "pkgs/noip2/noip2"
...
clean:
	...
	-rm -rf $(OBJ_DIR) $(OBJ_DIR_O3) $(DIST).bin $(DIST)d.bin *.dSYM ../$(DIST).tgz pas $(addprefix pru/pru_realtime.,bin lst txt) web/edata_embed.c kiwi.gen.h $(DIST)d $(DIST)d.aout $(DIST)d_realtime.bin .comp_ctr
